...
for split in train valid test; do
  ...
  # Tokenize portuguese captions
  cut -f4 < ${DATA_PATH}/${PREFIX}.${split} \
    | $REPLACE_UNICODE_PUNCT | $NORM_PUNC -l pt \
    | $REM_NON_PRINT_CHAR | $TOKENIZER -no-escape -threads 8 -l pt \
    | $LOWER_REMOVE_ACCENT > ${DATA_PATH}/${split}.pt
done

# Apply the pre-learned BPE
for split in train valid test; do
  for lg in en pt; do
    if [ ! -f "${DATA_PATH}/${split}.en-pt.${lg}" ]; then
      cat ${DATA_PATH}/${split}.${lg} | fastBPE/fast applybpe_stream bpe/bpe50k.codes bpe/bpe50k.vocab \
        > "${DATA_PATH}/${split}.en-pt.${lg}" &
    fi
  done
done
wait
